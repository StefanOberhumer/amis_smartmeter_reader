...
void WebserverWsDataClass::wsClientRequest(AsyncWebSocketClient *client, char* requestData, size_t requestLength) {
    ...
    */
    ...
    // add space for 10 "additional" objects and some extra bytes
    DynamicJsonDocument root(JSON_OBJECT_SIZE(32) + JSON_OBJECT_SIZE(10) + 32);
    ...
    if (requestLength >= 15 && requestLength <= root.capacity()) { // '{"command":"x"}'
        ...
        error = deserializeJson(root, requestData, requestLength);
    }
    if (error) {
        /*
        TODO(anyone): sending
